const { InvoiceForPurchaseMessage } = require('../models')
...
const storage = multer.diskStorage({
    destination: (req, file, cb) => {
      const uploadDir = path.join(__dirname, '../uploads/invoiceforpurchase');
...
const fetchReplies = async (parentId) => {
    ...
    const replies = await InvoiceForPurchaseMessage.findAll({
...
module.exports = {
   // Fetch all root messages related to a user and their replies
	async getMessagesAndRepliesForUser  (req, res) {
        try {
            const AllMessages = await InvoiceForPurchaseMessage.findAll();
            const rootMessages = await InvoiceForPurchaseMessage.findAll({
              where: {
                [Op.or]: [
...
// Create a new reply
    async createReply (req, res) {
        try {
           ...
           await InvoiceForPurchaseMessage.create(newMessage);
            const newMessageContent = await InvoiceForPurchaseMessage.findAll();
