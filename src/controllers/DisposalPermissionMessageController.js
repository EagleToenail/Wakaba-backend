const { DisposalPermissionMessage } = require('../models')
...
const storage = multer.diskStorage({
    destination: (req, file, cb) => {
      const uploadDir = path.join(__dirname, '../uploads/disposalpermission');
...
module.exports = {
   // Fetch all root messages related to a user and their replies
	async getMessagesAndRepliesForUser  (req, res) {
        try {
            const AllMessages = await DisposalPermissionMessage.findAll();
            const rootMessages = await DisposalPermissionMessage.findAll({
              where: {
                [Op.or]: [
            ...
            const replies = await DisposalPermissionMessage.findAll({
              where: {
                parentMessageId: {
...
// Create a new reply
    async createReply (req, res) {
        try {
           ...
           await DisposalPermissionMessage.create(newMessage);
            const newMessageContent = await DisposalPermissionMessage.findAll();
